...
class Utils:
    def __init__(self, language, port, server_ip, registration_ip):
    ...
    def acquire_initial_state(self):
        ...
        with open(os.path.join(folder_path, "dialogue_state.json"), 'w') as f:
        ...
        with open(os.path.join(folder_path, "speakers_info.json"), 'w') as f:
            if self.language == "it":
                user_name = "Utente"
            else:
                user_name = "User"
        ...
        with open(os.path.join(folder_path, "dialogue_statistics.json"), 'w') as f:
    ...
    def add_speaker_statistics(self, new_speaker_info):
        ...
        with open(os.path.join(folder_path, "speakers_info.json"), 'r') as info:
            speakers_info = json.load(info)
        # Load the statistics about the interactions with existing users to update them
        with open(os.path.join(folder_path, "dialogue_statistics.json"), 'r') as stats:
        ...
        with open(script_folder + "/speakers_info.json", 'w') as f:
